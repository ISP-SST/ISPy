...
class deep_network(object):
    ...
    def __init__(self):
        ...
        DATA_PATH = os.path.join(this_dir, "../data/{0}_denoising_weights.hdf5".format(self.network_type))
...
# =================================================================================
def neural_network(input_data, niterations = 2, scale = None, plotOption = False, test_Option=False, split=False):
    """Run the denoising neural network algorithm.
    Arguments:
        input_data: data cube
        niterations: number of times the neural network, cleaning the residuals
        scale: normalization to match training values. The std of the normalized data has to be around 1e-3
        plotOption: some plots to check the quality of the denoising.
        test_Option: crop the input data to perform a quick check.
        split: run the network splitting the data into multiple partitions (for big files)
    ...
    if scale is not None and scale > 0.0:
        ...
        print('==> Scale factor (noise around 1e-3) = {}'.format(sc))
    ...
    for istokes in [1,2,3]:
        ...
        for jj in range(nt):
            ...
            input0 = input_data[jj,istokes,:,:int(input_data.shape[3]/4.)*int(4),:int(input_data.shape[4]/4.)*int(4)]*sc
            ...
            if plotOption is True:
                ...
                plt.imshow(input0[lambdai,:,:],cmap='seismic',origin='lower',interpolation='None',vmin=-medio,vmax=+medio)
                plt.subplot(132)
                plt.title('Clean image')
                plt.imshow(input0[lambdai,:,:]-ciclo[lambdai,:,:],cmap='seismic',vmin=-medio,vmax=+medio,origin='lower',interpolation='None')
                plt.subplot(133)
                plt.title('Difference')
                plt.imshow(ciclo[lambdai,:,:],cmap='seismic',vmin=-medio,vmax=+medio,origin='lower',interpolation='None')        
                plt.savefig('images/output_t'+str(numerotime)+'_i'+str(niterations)+'.pdf',bbox_inches='tight')
                ...
                if input0.shape[0] > 1.5:
                    ...
                    plt.title('Difference')
                    plt.imshow(ciclo[lambdai,:,:],cmap='seismic',vmin=-medio,vmax=+medio,origin='lower',interpolation='None')        
                    plt.savefig('images/outputB_t'+str(numerotime)+'_i'+str(niterations)+'.pdf',bbox_inches='tight')
            ...
            new_output[jj,istokes,:,:int(input_data.shape[3]/4.)*int(4),:int(input_data.shape[4]/4.)*int(4)] = output0
